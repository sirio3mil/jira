...
export class SprintService {
  ...
  async getIssuesBySprintID(
    ...
    const config = {
      url: `/rest/agile/1.0/board/${boardID}/sprint/${sprintID}/issue?expand=changelog&fields=changelog,id,key,summary,aggregatetimespent,assignee,status,issuetype,customfield_10106&startAt=${startAt}&jql=type in (standardIssueTypes())`,
    };
    return await lastValueFrom(
      this.httpService.get(config.url).pipe(
        map((response) => {
